...
//const float PI = 0;
//const float GyroMeasError = 0;     // gyroscope measurement error in rads/s (start at 60 deg/s), then reduce after ~10 s to 3
//const float beta = 0;  // compute beta
//const float GyroMeasDrift = 0;      // gyroscope measurement drift in rad/s/s (start at 0.0 deg/s/s)
//const float zeta = 0;  // compute zeta, the other free parameter in the Madgwick scheme usually set to a small or zero value
...
//	HAL_I2C_Master_Transmit(&hi2c2, address, &data_write[0], 2, 10);
//}
...
//	HAL_I2C_Master_Transmit(&hi2c2, address, &data_write[0], 1, 10);
//   // i2c.read(address, data, 1, 0);
//HAL_I2C_Master_Receive(&hi2c2, address,&data[0], 1,0);
//    return data[0]; 
//}
...
//	HAL_I2C_Master_Receive(&hi2c2, address,&data[0], count,0);
